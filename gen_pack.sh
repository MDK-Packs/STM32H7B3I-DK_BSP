...
"${PACKCHK}" "${PACK_BUILD}/${PACK_VENDOR}.${PACK_NAME}.pdsc" -i "${CMSIS_PACK_ROOT}/.Web/Keil.STM32H7xx_DFP.pdsc" -i "${CMSIS_PACK_ROOT}/.Web/ARM.CMSIS.pdsc" -i "${CMSIS_PACK_ROOT}/.Web/ARM.CMSIS-Driver.pdsc" -n PackName.txt
errorlevel=$?
if [ $errorlevel -ne 0 ]; then
